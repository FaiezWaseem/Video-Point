    ...
    firebase
      ...
      .on("child_added", function (snapshot) {
        if (snapshot.val().uid === uid) {
          firebase
            .database()
            .ref("video/all/" + snapshot.val().key)
            .update({
              profile: avatar,
            });
          ...
          firebase
            .database()
            .ref("video/" + snapshot.val().type + "/" + snapshot.val().key)
            .update({
              profile: avatar,
            });
        }
...
upload.onclick = function () {
  ...
  var uploadTask = firebase
    .storage()
    .ref("Profile/" + img + rand(99999) + ".png")
    .put(files[0]);
...
function myVideos() {
  ...
  firebase
    ...
    .on("child_added", function (snapshot) {
      ...
      ul.innerHTML += `          <div class="video" data-id="${
        snapshot.key
      }" onclick="videoClicked(this)">
        ...
        <div class="video__details">
          ...
          <div class="title">
            ...
            <span>${snapshot.val().view}view • ${convertTime(
        snapshot.val().time
      )}</span>
          </div>
        </div>
...
function DBUpload(url) {
  ...
  try {
    ...
    firebase
      .database()
      .ref("video/all/" + newPostKey)
      .set(obj);
    firebase
      .database()
      .ref("video/" + type + "/" + newPostKey)
      .set(obj);
    firebase
      .database()
      .ref("Userposts/" + uid + "/" + newPostKey)
      .set(obj);
    get(".Loading-Modal").style.display = "none";
...
function uploadThumbnail(file) {
  const id =
    "AKfycbwVJszGTGgfWl5Pexu8YrG010DMcJQVaDmyYqivoedIvm83ZTmi1rAdmKbGD6h8r5bu1g";
  const url = `https://script.google.com/macros/s/${id}/exec`;
  console.log("uploading Thumbnail");
  const qs = new URLSearchParams({
    filename: `img${rand(100) + rand(789)}`,
    mimeType: "image/jpeg",
  });
  fetch(`${url}?${qs}`, {
    method: "POST",
...
function getFileShaingPermission(fid) {
  const id =
    "AKfycbyb3Z3EwstJO5cKa8k5cIMglSRo4kg2mf1VVDdL8-evCQ4M063HPLUlw_L7f9S2JE4";
  const url = `https://script.google.com/macros/s/${id}/exec`;
  ...
  function resumableUpload(e) {
    ...
    ru.Do(resource, function (res, err) {
      ...
      try {
        ...
        document.querySelector(".Loading-Modal h1").innerText =
